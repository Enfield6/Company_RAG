...
4. 根据引用编号、图片说明和回答主题，把图片重新编排到最相关的回答段落之后；
5. 可展开的引用原文和相似度。

系统只继承原文中的图片语义关系，不照搬原文版式和图片顺序。图片会标注“直接命中”或“相关界面”，并以正文内的说明卡片呈现；多张图片同时服务于一个步骤时会组合成紧凑图组。完整结构保存在消息的 `content_blocks` 中，重新打开历史对话仍会保持重新编排后的版式。
