...
2. Qwen 原生增量回答文字；
3. 持续更新的安全图文结构快照；
4. 直接命中的文档图片，或距离引用段落不超过 12 个元素的同段图片；图片只有明确命中带来源标记的回答步骤时才会出现；
5. 可展开的引用原文和相似度。

系统只继承原文中的图片语义关系，不照搬原文版式和图片顺序。图片会标注“原文图片”或“同段图片”，紧跟在对应教程步骤之后；无法明确匹配步骤的图片不会追加到文章末尾。多张图片同时服务于一个步骤时会组合成紧凑图组。完整结构保存在消息的 `content_blocks` 中，重新打开历史对话仍会保持重新编排后的版式。

回答策略不是“找不到原文就拒绝”。公司制度、内部流程和产品配置等专属事实仍以知识库为准；低相关检索结果会被过滤，资料不足时模型可以明确区分“知识库事实、通用判断和建议”后继续回答。合理的日常或非工作问题也可以正常交流，不会被强行套上公司引用。除纯闲聊和极简单问题外，助手会结合用户可能的真实目的，补充 1 到 3 条可执行的后续建议。
...
| `IMAGE_ENRICHMENT_CONCURRENCY` | `4` | 图片理解并发数；可按百炼配额在 `1` 到 `8` 之间调整 |
...
| `RETRIEVAL_MIN_SCORE` | `0.4` | 最低检索相关度；低于此值时转为通用助手回答，避免把无关公司文档硬塞给模型 |
...
PDF 解析会剔除同页高频重复的透明遮罩、切片和装饰资源，并对剩余业务图片去重；图片理解默认以 4 路受控并发执行，避免含大量图片的文档逐张串行等待。若百炼账号并发额度较低，可通过 `IMAGE_ENRICHMENT_CONCURRENCY` 调低。
